 ...
 * 	（由于删除操作较繁琐（详见com.xjy.basic.RedBlackTree），以下代码尚未实现。如果删除操作不是很频繁，实际中可以使用“懒惰删除”，将key对应的value设置为null，节约时间成本）
 * @author Mr.Xu
...
public class RedBlackBST <Key extends Comparable<? super Key> , Value>{
	...
	public Node put(Key k , Value v, Node p) {
		...
		//应对三种变化，保持红黑树的平衡（自底向上实现平衡）
		if(isRed(p.right) && !isRed(p.left)) { //右红链接不允许存在（统一规范便于代码实现），进行左旋
			rotateLeft(p);
		}
		if(isRed(p.left) && isRed(p.left.left)) {//连续两条红左链接不允许存在，进行右旋
			rotateRight(p);
		}
		if(isRed(p.left) && isRed(p.right)) {//上面的情况右旋后会形成左右红链接，此时变色操作（对应2-3树中把中间节点挤上去，左右节点分离）
			flipColors(p);
		}
		//调整高度（插入新节点改变size，调整高度必不可少）
